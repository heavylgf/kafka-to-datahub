...
public class KafkaConsumerMessageRunnable {

    private static final Logger Logger = LoggerFactory.getLogger(KafkaConsumerMessageRunnable.class);
    ...
    static {
        ...
        properties.put("group.id", propertyUtils.getProperty("group.id"));
        // 自动提交
    ...
    public static void main(String[] args) {
        ...
        for (int i =0; i <= 5; i++) {
            new Thread(
//                new MyRunnable(filePropertiesMapModelList, projectName, datahubClient)).start();
                    new MyRunnable(filePropertiesMapModelList, projectName)).start();
        }
    ...
    static class MyRunnable implements Runnable {
        ...
        private List<FilePropertiesMapModel> filePropertiesMapModelList;
        private String projectName;
        private DatahubClient datahubClient;

        public MyRunnable(List<FilePropertiesMapModel> filePropertiesMapModelList, String projectName
//                , DatahubClient datahubClient
        ) {
            this.filePropertiesMapModelList = filePropertiesMapModelList;
...
//            this.datahubClient = datahubClient;
        ...
        public void run() {
            try {
                while (true) {
                    consumerMessageAutoCommit(filePropertiesMapModelList, projectName);                }
            }catch (Exception e){
                e.printStackTrace();
            }
        }
